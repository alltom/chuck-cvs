...
The OSC\_Recv class listens for incoming OSC packets on the specified port. Each instance of OSC\_Recv can create OSC\_Addr objects using its event() method to listen for packets at any valid OSC Address pattern.

An OSC\_Addr object can then be ChucKed to now to wait for messages to arrive, after which the nextMesg() and get\{Float\textbar String\textbar Int\}() methods can be used to fetch message data.
