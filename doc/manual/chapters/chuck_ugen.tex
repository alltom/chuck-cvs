...
\item {\bf .chan} - () - {\it input to channel N on the device (0 -\textgreater N-1)}
\item {\bf .channels} - (int, READ only) - returns the number of channels open on device
\end{chuckitemize}
}
...
\item {\bf .chan} - () - {\it output to channel N on the device (0 -\textgreater N-1)}
\item {\bf .channels} - (int, READ only) - returns the number of channels open on device
\end{chuckitemize}
...
\ugen{ Blit}
\begin{chuckitemize}
\item band limited pulse generator - can set the value of the current sample
\item default for each sample is 0 if not set
\end{chuckitemize}
\control
\begin{chuckitemize}
...
\item {\bf .chunks} - (int, READ/WRITE) - {\it the number of frames loaded per sample, must be set before a .read {\it (default 256)}}
\end{chuckitemize}
}
...
//\item {\bf .phase\underline{ }offset}  (float, READ/WRITE)  {\it phase offset}
\item {\bf .phase}  (float, READ/WRITE)  {\it current phase}
\item {\bf .sync}  (int, READ/WRITE) {\it (0) input modulates frequency, (1) (reserved), (2) input modulates phase}
\end{chuckitemize}
}
...
\item {\bf .pan} (float, READ/WRITE) {\it pan location value (0 - 1)}
\end{chuckitemize}
}
...
\item {\bf .duration} (dur, READ/WRITE) {\it duration of steady state}
...
\control
\begin{chuckitemize}
\item {\bf .openJoystick} (int, READ/WRITE) {\it set port to receive}
\item {\bf .recv} (HidMsg, READ only) {\it receives HID input}
\end{chuckitemize}
(see events tutorial)
}
...
\control
\begin{chuckitemize}
\item {\bf .type} (int, READ/WRITE) {\it type of data (member variable)}
\item {\bf .which} (int, READ/WRITE) {\it type of data (member variable)}
