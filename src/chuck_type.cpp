...
struct t_Type_ t_string = { te_string, "string", &t_object, sizeof(void *) };
...
//-----------------------------------------------------------------------------
// name: struct t_Namespace_Info_
// desc: namespace information
//-----------------------------------------------------------------------------

// constructor
t_Namespace_Info_::t_Namespace_Info_( S_Symbol s )
{
    name = s;
    name2value = S_empty();
    parent = NULL;
...
// add value
t_CKBOOL t_Namespace_Info_::add_value( t_Namespace_Value value )
{
    if( !value->name || !strcmp( S_name(value->name), "" ) )
        return FALSE;

    // look
    t_Namespace_Value val = (t_Namespace_Value)S_look( name2value, value->name );
    if( val )
        return FALSE;

    // add it
    S_enter( name2value, value->name, value );
...
// get value
t_Namespace_Value t_Namespace_Info_::get_value( S_Symbol name )
{
    if( !name || !strcmp( S_name(name), "" ) )
        return NULL;

    // look
    return (t_Namespace_Value)S_look( name2value, name );
}
